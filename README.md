...
| **다중 프로필** | 여러 S3 계정/버킷을 프로필로 저장, 즉시 전환 |
| **프로필 Import/Export** | 암호화 프로필 파일 Import/Remove 중심 운영, 생성·수정은 권한 모델로 제한 |
...
| 역할 | Import | Remove | Create | Edit | Purge | Secrets 관리 |
|------|:------:|:------:|:------:|:----:|:-----:|:------------:|
| admin | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| operator | ✓ | ✓ | | | ✓ | |
| viewer | ✓ | | | | | |
...
PurgePolicy {
  ...
  batch: {
    ...
    warningThreshold:           5000              // TODO: 고객 확인
    notRecommendedThreshold:    10000             // TODO: 고객 확인
  }
}
```
...
Rust 테스트에는 `build_cdn_url` 정규화 검증, `hyosung` NotImplemented 확인 등이 포함됩니다.
...
| Purge 대용량 임계값 | TODO | 5,000개 경고 / 10,000개 미권고 기준 → 고객 확인 필요 |
