...
| Header/Metadata 수동 입력 | Content-Type, Cache-Control 외 사용자 정의 Header와 Metadata 입력 지원 |
| 자동 Metadata | 확장자/Profile 정책 기반 Metadata 자동 적용 |
| Metadata 실패 재시도 | Header/Metadata 적용 실패 항목은 수동 재시도 가능 구조 제공 |
...
| 기본 모드 | 기본값은 수동 Purge |
| 자동 Purge 옵션 | 자동 Purge 여부 체크 버튼 제공 |
| 자동 Purge 승인 | 자동 Purge 실행 전 승인 팝업 제공 |
| 대용량 Purge 경고 | 대량 Purge 실행 전 추가 경고 제공 |
| Purge 범위 | 전체/개별/일부 Purge 지원 |
| 정책 연계 | Overwrite/Skip 정책에 따라 Purge 대상 결정 |
| UI 위치 | 자동 Purge 버튼은 미리보기 영역 근처에 배치 |
...
| 항목 | 기준 |
| --- | --- |
| 기본 Batch Size | 1,000개 |
| 경고 기준 | 5,000개 이상 |
| 미권고 경고 기준 | 10,000개 이상 |
| 확정 상태 | 고객 확인 필요 TODO로 관리 |
| 설정 구조 | Profile 또는 설정에 Batch Size 정책 저장 가능 구조 |
...
| 정책 | Purge 기본 모드, 승인 필요 여부, Batch Size, 대용량 경고 기준 |
| 정책 | Upload Overwrite/Skip 정책 |
| Metadata | 기본 Content-Type, Cache-Control, 사용자 정의 Header, 사용자 Metadata |
| 로그 적재 | 고객 제공 S3 Log Bucket / Prefix |
| 권한 | 일반 사용자 Import/Remove 중심 권한, 관리자 생성/수정 권한 |
| 범위 | 프로젝트 단위/Profile, 사용자 단위/Profile 구분 |
| CDN | 하나의 Profile 내 복수 CDN Provider 구성 가능 |
| 인증 | AI LB 인증키/세션 또는 외부 인증 시스템 연동 참조 |
| 정책 | Default Cache-Control |
| 정책 | Content-Type Override |

Secret 정보는 평문 저장을 피하고, 기존 프로젝트의 보안 저장 방식 또는 OS Keyring 방식을 따릅니다.

일반 사용자는 Profile 생성/수정보다 암호화 Profile Import/Remove 중심으로 사용합니다. Profile 생성/수정 및 Secret 관리 기능은 권한 모델로 제한합니다.
...
| AI LB 연동 | AI LB 인증키/세션을 참조할 수 있는 Stub 구조 |
...
| 고객 S3 로그 적재 | 고객 제공 S3 Bucket/Prefix로 JSON 로그 적재 구조 |
| 로그 적재 대상 | 업로드/다운로드/Purge/오류/Metadata 실패 |
| 로그 적재 재시도 | 실패한 로그 적재 항목 재시도 구조 |
| 최근 로그 | 최근 작업 로그 조회 |
...
| --- | --- |
| 테스트 버전 | 설치형 패키지 기준 |
| Windows Server | Windows Server 2025/2022/2019 지원 가능 여부 TODO |
| Desktop OS | Windows/macOS/Linux 최근 3개년 지원 범위 TODO |
| Unix 서버 | Unix 서버 지원 가능 여부 TODO |
| 자동 패치 | 자동 패치/보안 업데이트 구조 TODO |
| 무재시작 패치 | 재시작 없는 패치 가능 여부는 확정하지 않고 검토 항목으로만 문서화 |
