...
impl S3Adapter {
        ...
        E: ProvideErrorMetadata,
    {
        ...
        let user_message = if code == "SignatureDoesNotMatch" {
            "Secret Access Key ????????????????? ??????? ??????? ???????????????? ??????? Access Key/Secret??????? AWS CLI PutObject??????????? ????????????????"
        } else {
            operation
        };
        tracing::error!(
        ...
        anyhow::anyhow!(
            "S3 {} ???????: {} (access_key_id={}, region={}, bucket={}, key={}, api={}, aws_error_code={})",
            operation,
            user_message,
            mask_access_key_id(&self.creds.access_key_id),
            self.region,
    ...
    async fn signed_get(&self, url: &Url) -> Result<reqwest::Response> {
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP GET ???????"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP GET ???????")
    }
    ...
    async fn signed_head(&self, url: &Url) -> Result<reqwest::Response> {
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP HEAD ???????"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP HEAD ???????")
    }
    ...
    async fn signed_put(
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP PUT ???????"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP PUT ???????")
    }
    ...
    async fn signed_post(
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP POST ???????"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP POST ???????")
    }
    ...
    async fn signed_delete(&self, url: &Url) -> Result<reqwest::Response> {
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP DELETE ???????"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP DELETE ???????")
    }
    ...
    pub async fn test_connection(&self, base_prefix: &str) -> Result<Vec<String>> {
        ...
        if let Err(err) = self.test_list_objects_v2(&prefix).await {
            ...
            if message.contains("AccessDenied") || message.contains("HTTP 403") {
                warnings.push(format!("?????? ?????? ?????? ???????: {}", message));
            } else {
                warnings.push(format!("ListObjectsV2 ??????: {}", message));
            }
        }

        let put_ok = match self.test_put_object(&test_key).await {
            Ok(()) => true,
            Err(err) => {
                warnings.push(format!("PutObject ??????(?????? ?????? ???): {}", err));
                false
            }
        };
        ...
        if put_ok {
            if let Err(err) = self.test_delete_object(&test_key).await {
                warnings.push(format!("DeleteObject ??????: {}", err));
            }
        }
    ...
    async fn test_sts_get_caller_identity(&self) -> Result<()> {
        ...
        let url = Url::parse(&self.sts_url()).context("STS URL ??????? ???????")?;
        ...
        let resp = req
            .send()
            .await
            .context("STS GetCallerIdentity ???????: HTTP ??????? ???????")?;
    ...
    async fn test_head_bucket(&self) -> Result<()> {
        ...
        let url = Url::parse(&self.bucket_url()).context("HeadBucket URL ??????? ???????")?;
        let resp = self.signed_head(&url).await.context(format!(
            "S3 HeadBucket ???????: bucket={}, region={}",
    ...
    async fn test_list_objects_v2(&self, prefix: &str) -> Result<()> {
        ...
        .context("ListObjectsV2 URL ??????? ???????")?;
        let resp = self.signed_get(&url).await.context(format!(
            "S3 ListObjectsV2 ???????: bucket={}, key={}, region={}",
    ...
    async fn test_put_object(&self, key: &str) -> Result<()> {
        ...
        let resp = self
            ...
            .await
            .context(format!(
                "S3 PutObject ???????: bucket={}, key={}, region={}",
    ...
    async fn test_delete_object(&self, key: &str) -> Result<()> {
        ...
        let url = Url::parse(&format!("{}/{}", self.bucket_url(), encode_key(key)))
            .context("DeleteObject URL ??????? ???????")?;
        let resp = self.signed_delete(&url).await.context(format!(
            "S3 DeleteObject ???????: bucket={}, key={}, region={}",
            self.bucket, key, self.region
        ))?;
    ...
    async fn list_objects_page(
        ...
        let url = Url::parse(&raw).context("URL ??????? ???????")?;
        ...
        for attempt in 0u32..3 {
            match self.signed_get(&url).await {
                Err(e) if attempt < 2 => {
                    tracing::warn!("?????? ?????? ?????????????? ??????? ?????????{}/3: {}", attempt + 1, e);
                    tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                    delay_ms *= 2;
                    continue;
                }
                Err(e) => return Err(e.context("HTTP GET ???????")),
                Ok(resp) => {
                    // C-3 + H-4: HTTP ??????? ??????? ????????? ?????????????????????????? ??????????????? ???????
                    let status = resp.status();
                    if status.is_success() {
                    ...
                    if attempt < 2 && is_retryable_status(code) {
                        tracing::warn!("?????? ?????? ?????????{}/3: HTTP {}", attempt + 1, code);
                        tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                        delay_ms *= 2;
                        continue;
                    }
                    let body = resp.text().await.unwrap_or_default();
                    return Err(anyhow::anyhow!("S3 ?????? ?????? ??????? (HTTP {}): {}", code, body));
                }
            }
        }
        unreachable!()
    ...
    pub async fn head_object_etag(&self, key: &str) -> Result<Option<String>> {
        ...
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("HeadObject ???????: HTTP {}", resp.status()));
        }
    ...
    pub async fn delete_objects(&self, keys: &[String]) -> Result<Vec<String>> {
        ...
        let resp = req.send().await.context("HTTP POST(delete) ???????")?;
        if !resp.status().is_success() {
    ...
    pub async fn put_object(&self, key: &str, data: Vec<u8>, content_type: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("PutObject ???????: HTTP {}", resp.status()));
        }
        Ok(())
    }
    ...
    pub async fn download_with_cancel(
        ...
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("GetObject ???????: HTTP {}", resp.status()));
        }
        ...
        while let Some(chunk) = stream.next().await {
            ...
            file.write_all(&chunk).await.context("??????? ??????? ???????")?;
    ...
    pub async fn rename_object(&self, src_key: &str, dst_key: &str) -> Result<()> {
        ...
        let resp = req.send().await.context("CopyObject HTTP PUT ???????")?;
        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("CopyObject ???????: {}", body));
        }
    ...
    pub async fn presign_get(&self, key: &str, expires_in_seconds: u64) -> Result<String> {
        ...
        let url = Url::parse(&raw).context("Presign URL ??????? ???????")?;
        let canonical_query = {
    ...
    async fn upload_single(
        ...
        let data = fs::read(local_path).await.context("??????? ??????? ???????")?;
        if !on_progress(0, total) {
            return Err(anyhow::anyhow!("???????????????????????????????"));
        }
    ...
    /// ?????????????? ???????????????: ??????? 4?????????????????????????????????
    /// ??????? ???????????????????= MAX_CONCURRENT_PARTS ?? PART_SIZE = 40 MB
    ...
    async fn initiate_multipart_upload(
        ...
        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("InitiateMultipartUpload ???????: {}", body));
        }

        let text = resp.text().await.context("??????? ??????? ???????")?;
        xml_extract(&text, "UploadId").context("UploadId ??????? ???????")
    }
    ...
    async fn upload_part(
        ...
        for attempt in 0u32..3 {
            ...
            match req.send().await {
                Err(e) if attempt < 2 => {
                    tracing::warn!("??????? ??????????????????????? ??????? ?????????{}/3 (part {}): {}", attempt + 1, part_number, e);
                    tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                    delay_ms *= 2;
                    continue;
                }
                Err(e) => return Err(anyhow::anyhow!("HTTP PUT(part) ???????: {}", e)),
                Ok(resp) => {
                    let status = resp.status();
                    if status.is_success() {
                        return resp
                            ...
                            .and_then(|v| v.to_str().ok())
                            .map(|e| e.trim_matches('"').to_owned())
                            .context("UploadPart ETag ??????? ???????");
                    }
                    let code = status.as_u16();
                    if attempt < 2 && is_retryable_status(code) {
                        tracing::warn!("??????? ??????????????????{}/3 (part {}): HTTP {}", attempt + 1, part_number, code);
                        tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                        delay_ms *= 2;
                        continue;
                    }
                    let body = resp.text().await.unwrap_or_default();
                    return Err(anyhow::anyhow!("UploadPart ??????? (part {}): {}", part_number, body));
                }
            }
        }
        unreachable!()
    ...
    async fn complete_multipart_upload(
        ...
        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("CompleteMultipartUpload ???????: {}", body));
        }

        let text = resp.text().await.context("??????? ??????? ???????")?;
        xml_extract(&text, "ETag")
            .map(|e| e.trim_matches('"').to_owned())
            .context("CompleteMultipartUpload ETag ??????? ???????")
    }
...
impl StorageAdapter for S3Adapter {
    ...
    async fn upload_file(
        ...
        let local_str = local.to_str().context("??????????????? ???????? ??????? ??????")?;
        // tx.send() ???????????????? ?????????????????? ???????????????? ???????
