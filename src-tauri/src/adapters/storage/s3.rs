...
/// ???¨Í∏∞ ?¥ÏÉÅ?¥Î©¥ Î©Ä?∞Ìåå???ÖÎ°ú?úÎ°ú ?ÑÌôò
pub const MULTIPART_THRESHOLD: u64 = 10 * 1024 * 1024; // 10 MB
/// ?åÌä∏???¨Í∏∞ (S3 ÏµúÏÜå 5 MB, ÎßàÏ?Îß??åÌä∏ ?úÏô∏)
pub const PART_SIZE: usize = 10 * 1024 * 1024; // 10 MB
/// ?ôÏãú ?åÌä∏ ?ÖÎ°ú????
const MAX_CONCURRENT_PARTS: usize = 4;

// ?Ä?Ä?Ä S3Adapter ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
impl S3Adapter {
        ...
        E: ProvideErrorMetadata,
    {
        ...
        let user_message = if code == "SignatureDoesNotMatch" {
            "Secret Access Key Î∂àÏùºÏπ??êÎäî ?úÎ™Ö ?ùÏÑ± ?§Î•ò?ÖÎãà?? Í∞ôÏ? Access Key/Secret?ºÎ°ú AWS CLI PutObjectÎ•?Î®ºÏ? ?ïÏù∏?òÏÑ∏??"
        } else {
            operation
        };
        tracing::error!(
        ...
        anyhow::anyhow!(
            "S3 {} ?§Ìå®: {} (access_key_id={}, region={}, bucket={}, key={}, api={}, aws_error_code={})",
            operation,
            user_message,
            mask_access_key_id(&self.creds.access_key_id),
            self.region,
    ...
    // ?Ä?Ä Signed HTTP Helpers ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
    ...
    async fn signed_get(&self, url: &Url) -> Result<reqwest::Response> {
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP GET ?§Ìå®"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP GET ?§Ìå®")
    }
    ...
    async fn signed_head(&self, url: &Url) -> Result<reqwest::Response> {
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP HEAD ?§Ìå®"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP HEAD ?§Ìå®")
    }
    ...
    async fn signed_put(
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP PUT ?§Ìå®"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP PUT ?§Ìå®")
    }
    ...
    async fn signed_post(
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP POST ?§Ìå®"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP POST ?§Ìå®")
    }
    ...
    async fn signed_delete(&self, url: &Url) -> Result<reqwest::Response> {
        ...
        for attempt in 0..3 {
            ...
            match req.send().await {
                ...
                Err(err) => return Err(err).context("HTTP DELETE ?§Ìå®"),
            }
        }
        Err(last_err.expect("retry error")).context("HTTP DELETE ?§Ìå®")
    }

    // ?Ä?Ä Public Operations ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
    ...
    pub async fn test_connection(&self, base_prefix: &str) -> Result<Vec<String>> {
        ...
        if let Err(err) = self.test_head_bucket().await {
            ...
            match self.test_get_bucket_location().await {
                ...
                Err(location_err) => warnings.push(format!(
                    "HeadBucket/GetBucketLocation Í≤ΩÍ≥†: {} / {}",
        ...
        if let Err(err) = self.test_list_objects_v2(&prefix).await {
            ...
            if message.contains("AccessDenied") || message.contains("HTTP 403") {
                warnings.push(format!("Î™©Î°ù Ï°∞Ìöå Í∂åÌïú ?ÜÏùå: {}", message));
            } else {
                warnings.push(format!("ListObjectsV2 Í≤ΩÍ≥†: {}", message));
            }
        }
        ...
        if let Err(err) = self.test_delete_object(&test_key).await {
            warnings.push(format!("DeleteObject Í≤ΩÍ≥†: {}", err));
        }
    ...
    async fn test_sts_get_caller_identity(&self) -> Result<()> {
        ...
        let url = Url::parse(&self.sts_url()).context("STS URL ?ùÏÑ± ?§Ìå®")?;
        ...
        let resp = req
            .send()
            .await
            .context("STS GetCallerIdentity ?§Ìå®: HTTP ?îÏ≤≠ ?§Ìå®")?;
    ...
    async fn test_head_bucket(&self) -> Result<()> {
        ...
        let url = Url::parse(&self.bucket_url()).context("HeadBucket URL ?ùÏÑ± ?§Ìå®")?;
        let resp = self.signed_head(&url).await.context(format!(
            "S3 HeadBucket ?§Ìå®: bucket={}, region={}",
    ...
    async fn test_get_bucket_location(&self) -> Result<()> {
        let url = Url::parse(&format!("{}/?location", self.bucket_url()))
            .context("GetBucketLocation URL ?ùÏÑ± ?§Ìå®")?;
        let resp = self.signed_get(&url).await.context(format!(
            "S3 GetBucketLocation ?§Ìå®: bucket={}, region={}",
            self.bucket, self.region
        ))?;
    ...
    async fn test_list_objects_v2(&self, prefix: &str) -> Result<()> {
        ...
        .context("ListObjectsV2 URL ?ùÏÑ± ?§Ìå®")?;
        let resp = self.signed_get(&url).await.context(format!(
            "S3 ListObjectsV2 ?§Ìå®: bucket={}, key={}, region={}",
    ...
    async fn test_put_object(&self, key: &str) -> Result<()> {
        ...
        let resp = self
            ...
            .await
            .context(format!(
                "S3 PutObject ?§Ìå®: bucket={}, key={}, region={}",
    ...
    async fn test_delete_object(&self, key: &str) -> Result<()> {
        ...
        let url = Url::parse(&format!("{}/{}", self.bucket_url(), encode_key(key)))
            .context("DeleteObject URL ?ùÏÑ± ?§Ìå®")?;
        let resp = self.signed_delete(&url).await.context(format!(
            "S3 DeleteObject ?§Ìå®: bucket={}, key={}, region={}",
            self.bucket, key, self.region
        ))?;
    ...
    async fn verify_list_access(&self) -> Result<()> {
        ...
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("Î≤ÑÌÇ∑ ?ëÍ∑º ?§Ìå®: HTTP {}", resp.status()));
        }
        Ok(())
    }

    /// ?®Ïùº ?òÏù¥ÏßÄ Î™©Î°ù Ï°∞Ìöå (?¥Î???
    async fn list_objects_page(
        ...
        let url = Url::parse(&raw).context("URL ?åÏã± ?§Ìå®")?;

        // M-5: ÏßÄ??Î∞±Ïò§???¨Ïãú??(ÏµúÎ? 3??
        let mut delay_ms = 500u64;
        for attempt in 0u32..3 {
            match self.signed_get(&url).await {
                Err(e) if attempt < 2 => {
                    tracing::warn!("Î™©Î°ù Ï°∞Ìöå ?§Ìä∏?åÌÅ¨ ?§Î•ò ?¨Ïãú??{}/3: {}", attempt + 1, e);
                    tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                    delay_ms *= 2;
                    continue;
                }
                Err(e) => return Err(e.context("HTTP GET ?§Ìå®")),
                Ok(resp) => {
                    // C-3 + H-4: HTTP ?ÅÌÉú ?ïÏù∏ ???§Î•ò ?ëÎãµ??Îπ?Î™©Î°ù?ºÎ°ú ?§Ïù∏?òÏ? ?äÏùå
                    let status = resp.status();
                    if status.is_success() {
                        let text = resp.text().await.context("?ëÎãµ ?ΩÍ∏∞ ?§Ìå®")?;
                        return parse_list_response(&text);
                    }
                    let code = status.as_u16();
                    if attempt < 2 && is_retryable_status(code) {
                        tracing::warn!("Î™©Î°ù Ï°∞Ìöå ?¨Ïãú??{}/3: HTTP {}", attempt + 1, code);
                        tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                        delay_ms *= 2;
                        continue;
                    }
                    let body = resp.text().await.unwrap_or_default();
                    return Err(anyhow::anyhow!("S3 Î™©Î°ù Ï°∞Ìöå ?§Ìå® (HTTP {}): {}", code, body));
                }
            }
        }
        unreachable!()
    ...
    /// C-3: ?ÑÏ≤¥ ?òÏù¥ÏßÄÎ•??úÌöå??1000Í∞?Ï¥àÍ≥º ?§Î∏å?ùÌä∏Î•?Î™®Îëê Î∞òÌôò
    ...
    pub async fn head_object_etag(&self, key: &str) -> Result<Option<String>> {
        ...
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("HeadObject ?§Ìå®: HTTP {}", resp.status()));
        }
    ...
    pub async fn delete_objects(&self, keys: &[String]) -> Result<Vec<String>> {
        ...
        let resp = req.send().await.context("HTTP POST(delete) ?§Ìå®")?;
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!(
                "DeleteObjects ?§Ìå®: HTTP {}",
                resp.status()
            ));
        }
    ...
    pub async fn put_object(&self, key: &str, data: Vec<u8>, content_type: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("PutObject ?§Ìå®: HTTP {}", resp.status()));
        }
        Ok(())
    }

    /// ?åÏùº ?ÖÎ°ú?? 10 MB ?¥ÏÉÅ?Ä ?êÎèô?ºÎ°ú Î©Ä?∞Ìåå???ÖÎ°ú??
    /// on_progress(transferred, total) ÏΩúÎ∞±?ºÎ°ú ÏßÑÌñâÎ•??ÑÎã¨
    ...
    pub async fn download_with_cancel(
        ...
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("GetObject ?§Ìå®: HTTP {}", resp.status()));
        }
        ...
        while let Some(chunk) = stream.next().await {
            if is_cancelled() {
                return Err(anyhow::anyhow!("?ëÏóÖ??Ï∑®ÏÜå?òÏóà?µÎãà??));
            }
            let chunk = chunk.context("?§Ïö¥Î°úÎìú ?§Ìä∏Î¶??§Î•ò")?;
            file.write_all(&chunk).await.context("?åÏùº ?∞Í∏∞ ?§Ìå®")?;
            received += chunk.len() as u64;
            on_progress(received, total);
        }

        file.flush().await.context("?åÏùº flush ?§Ìå®")?;
        Ok(())
    }

    /// S3 ?§Î∏å?ùÌä∏ ?¥Î¶Ñ Î≥ÄÍ≤?(CopyObject ??DeleteObject)
    pub async fn rename_object(&self, src_key: &str, dst_key: &str) -> Result<()> {
        ...
        let resp = req.send().await.context("CopyObject HTTP PUT ?§Ìå®")?;
        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("CopyObject ?§Ìå®: {}", body));
        }

        self.delete_objects(&[src_key.to_owned()])
            .await
            .context("?êÎ≥∏ ?§Î∏å?ùÌä∏ ??†ú ?§Ìå®")?;
        Ok(())
    }
    ...
    pub async fn presign_get(&self, key: &str, expires_in_seconds: u64) -> Result<String> {
        ...
        let url = Url::parse(&raw).context("Presign URL ?åÏã± ?§Ìå®")?;
        let canonical_query = {
    ...
    // ?Ä?Ä Multipart Upload Internals ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
    ...
    async fn upload_single(
        ...
        let data = fs::read(local_path).await.context("?åÏùº ?ΩÍ∏∞ ?§Ìå®")?;
        if !on_progress(0, total) {
            return Err(anyhow::anyhow!("?ëÏóÖ??Ï∑®ÏÜå?òÏóà?µÎãà??));
        }
    ...
    /// ?¨Îùº?¥Îî© ?àÎèÑ??Î∞©Ïãù: ÏµúÎ? 4Í∞??åÌä∏Î•??ôÏãú???ÖÎ°ú??
    /// ÏµúÎ? Î©îÎ™®Î¶??¨Ïö©??= MAX_CONCURRENT_PARTS √ó PART_SIZE = 40 MB
        ...
        loop {
            ...
            while let Some(result) = tasks.join_next().await {
                match result {
                    ...
                    Ok(Err(e)) => {
                        let _ = self.abort_multipart_upload(remote_key, &upload_id).await;
                        return Err(e.context("?åÌä∏ ?ÖÎ°ú???§Ìå®"));
                    }
                    Err(join_err) => {
                        let _ = self.abort_multipart_upload(remote_key, &upload_id).await;
                        return Err(anyhow::anyhow!("?åÌä∏ ?ÖÎ°ú???úÏä§???®Îãâ: {}", join_err));
                    }
                }
            }
        }

        // ?åÌä∏ Î≤àÌò∏ ?úÏúºÎ°??ïÎ†¨ ???ÑÎ£å
    ...
    async fn initiate_multipart_upload(
        ...
        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("InitiateMultipartUpload ?§Ìå®: {}", body));
        }

        let text = resp.text().await.context("?ëÎãµ ?ΩÍ∏∞ ?§Ìå®")?;
        xml_extract(&text, "UploadId").context("UploadId ?åÏã± ?§Ìå®")
    }
    ...
    async fn upload_part(
        ...
        for attempt in 0u32..3 {
            ...
            match req.send().await {
                Err(e) if attempt < 2 => {
                    tracing::warn!("?åÌä∏ ?ÖÎ°ú???§Ìä∏?åÌÅ¨ ?§Î•ò ?¨Ïãú??{}/3 (part {}): {}", attempt + 1, part_number, e);
                    tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                    delay_ms *= 2;
                    continue;
                }
                Err(e) => return Err(anyhow::anyhow!("HTTP PUT(part) ?§Ìå®: {}", e)),
                Ok(resp) => {
                    let status = resp.status();
                    if status.is_success() {
                        return resp
                            ...
                            .and_then(|v| v.to_str().ok())
                            .map(|e| e.trim_matches('"').to_owned())
                            .context("UploadPart ETag ?§Îçî ?ÜÏùå");
                    }
                    let code = status.as_u16();
                    if attempt < 2 && is_retryable_status(code) {
                        tracing::warn!("?åÌä∏ ?ÖÎ°ú???¨Ïãú??{}/3 (part {}): HTTP {}", attempt + 1, part_number, code);
                        tokio::time::sleep(tokio::time::Duration::from_millis(delay_ms)).await;
                        delay_ms *= 2;
                        continue;
                    }
                    let body = resp.text().await.unwrap_or_default();
                    return Err(anyhow::anyhow!("UploadPart ?§Ìå® (part {}): {}", part_number, body));
                }
            }
        }
        unreachable!()
    ...
    async fn complete_multipart_upload(
        ...
        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("CompleteMultipartUpload ?§Ìå®: {}", body));
        }

        let text = resp.text().await.context("?ëÎãµ ?ΩÍ∏∞ ?§Ìå®")?;
        xml_extract(&text, "ETag")
            .map(|e| e.trim_matches('"').to_owned())
            .context("CompleteMultipartUpload ETag ?åÏã± ?§Ìå®")
    }
    ...
    async fn abort_multipart_upload(&self, key: &str, upload_id: &str) -> Result<()> {
        ...
        // 404???¥Î? ?ÑÎ£å ?êÎäî Ï°¥Ïû¨?òÏ? ?äÏùå ??Î¨¥Ïãú
        if !resp.status().is_success() && resp.status().as_u16() != 404 {
            return Err(anyhow::anyhow!(
                "AbortMultipartUpload ?§Ìå®: {}",
                resp.status()
            ));
        }
        Ok(())
...
// ?Ä?Ä?Ä StorageAdapter Trait Impl ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
impl StorageAdapter for S3Adapter {
    ...
    async fn upload_file(
        ...
        let local_str = local.to_str().context("?†Ìö®?òÏ? ?äÏ? ?åÏùº Í≤ΩÎ°ú")?;
        // tx.send() ???∏Î∞î?¥Îìú Ï±ÑÎÑêÎ°??àÎ? Î∏îÎ°ú?πÎêòÏßÄ ?äÏùå
    ...
    async fn head_object(&self, key: &str) -> Result<ObjectMeta> {
        ...
        if resp.status().as_u16() == 404 {
            return Err(anyhow::anyhow!("?§Î∏å?ùÌä∏ ?ÜÏùå: {}", key));
        }
        if !resp.status().is_success() {
            return Err(anyhow::anyhow!("HeadObject ?§Ìå®: HTTP {}", resp.status()));
        }
...
// ?Ä?Ä?Ä XML Parsing ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
